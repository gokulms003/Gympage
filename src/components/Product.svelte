    ...
    <div class="flex flex-col gap-10 sm:gap-14 md:gap-24 flex-1 items-center justify-center pb-10 md:pb-14">
        ...
        {#each productFeatures as productFeature, index}
             <ProductCard productFeature={productFeature} {index}>
                {#if index === 0}
                Experience a begineer exercise <span class="text-indigo-400">master</span> class
                    {:else if index === 1}
                    Lay the foundation for <span class="text-indigo-400 font-medium">routine</span><br/>and consistency
                    {:else}
